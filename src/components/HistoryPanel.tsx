          ...
          {showFilters && (
            <div className="bg-white rounded-xl border border-purple-100 p-3 flex flex-wrap gap-3 animate-in">
              <div className="flex items-center gap-1.5">
                ...
                {[0, 1, 2, 3, 4, 5].map((v) => (
                  <button
                    key={v}
                    onClick={() => {
                      if (ratingMin === v && ratingMax === 5) {
                        setRatingMin(0)
                      } else if (ratingMin === 0 && ratingMax === v) {
                        setRatingMax(5)
                      } else {
                        if (ratingMin === 0) {
                          setRatingMin(v)
                ...
                <span className="text-[10px] text-stone-400">
                  {ratingMin === 0 && ratingMax === 5 ? '全部' : `${ratingMin}-${ratingMax}★`}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-xs text-stone-500">热量</span>
                <input type="number" placeholder="最低" value={calorieMin} onChange={(e) => setCalorieMin(e.target.value)}
                ...
                <input type="number" placeholder="最高" value={calorieMax} onChange={(e) => setCalorieMax(e.target.value)}
                  className="w-16 text-xs px-1.5 py-0.5 rounded-lg border border-stone-200 focus:outline-none focus:border-purple-300" />
                <span className="text-xs text-stone-400">千卡</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-xs text-stone-500">价格</span>
                <input type="number" placeholder="最低" value={costMin} onChange={(e) => setCostMin(e.target.value)}
                  className="w-16 text-xs px-1.5 py-0.5 rounded-lg border border-stone-200 focus:outline-none focus:border-purple-300" step="0.1" />
                <span className="text-xs text-stone-400">-</span>
                <input type="number" placeholder="最高" value={costMax} onChange={(e) => setCostMax(e.target.value)}
                  className="w-16 text-xs px-1.5 py-0.5 rounded-lg border border-stone-200 focus:outline-none focus:border-purple-300" step="0.1" />
              ...
              {(ratingMin > 0 || ratingMax < 5 || calorieMin || calorieMax || costMin || costMax || favoriteOnly) && (
                <button
                  onClick={() => {
                    setRatingMin(0); setRatingMax(5)
                    setCalorieMin(''); setCalorieMax('')
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => toggleFavorite(item.id)} className="transition-transform duration-150 hover:scale-125">
                      <Heart className={`w-4 h-4 ${item.favorite ? 'fill-red-400 text-red-400' : 'text-stone-300 hover:text-red-300'}`} />
                    </button>
                    <span className="font-semibold text-sm text-stone-800">{item.name}</span>
                  </div>
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} onClick={() => rateSandwich(item.id, star)} className="transition-transform duration-150 hover:scale-125">
                        <Star className={`w-4 h-4 ${star <= item.rating ? 'fill-amber-400 text-amber-400' : 'text-stone-300 hover:text-amber-300'}`} />
                      </button>
                    ))}
                  </div>
                </div>
                <div className="flex flex-wrap gap-1 mb-2">
                  {item.layers.map((id, i) => (
                    <span key={i} className="text-xs px-1.5 py-0.5 rounded-full font-medium"
                      style={{ backgroundColor: getIngredientColor(id) + '30', color: getIngredientColor(id) }}>
                ...
                {editingNote === item.id ? (
                  <div className="flex gap-1.5 mb-1">
                    <input type="text" value={noteDraft} onChange={(e) => setNoteDraft(e.target.value)}
                      onKeyDown={(e) => { if (e.key === 'Enter') handleSaveNote(item.id); if (e.key === 'Escape') setEditingNote(null) }}
                    ...
                    <button onClick={() => handleSaveNote(item.id)} className="text-xs px-2 py-1 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors font-medium">保存</button>
                  </div>
                ) : item.note ? (
                  <div className="mb-1 text-xs text-stone-500 italic bg-stone-50 px-2 py-1 rounded-lg">💬 {item.note}</div>
                ) : null}
                ...
                {isCopyPending && copyMissing.length > 0 && (
                  <div className="mb-1 p-2 rounded-xl bg-orange-50 border border-orange-200">
                    ...
                    <button onClick={() => handleCopyWithRestock(item.id)}
                      ...
                      <ShoppingCart className="w-3 h-3" />补货并继续
                    </button>
                  </div>
                )}

                <div className="flex items-center gap-2 pt-1 border-t border-stone-100">
                  <button onClick={() => handleCopy(item.id)} className="flex items-center gap-1 text-xs text-amber-600 hover:text-amber-800 transition-colors font-medium">
                    <Copy className="w-3 h-3" />再制一份
                  </button>
                  <button onClick={() => editingNote === item.id ? setEditingNote(null) : handleStartNote(item.id, item.note)}
                    className="flex items-center gap-1 text-xs text-stone-500 hover:text-stone-700 transition-colors">
                    <MessageSquare className="w-3 h-3" />{editingNote === item.id ? '取消' : '备注'}
                  </button>
                </div>
              </div>
            )
